...
function moveWidgetToToolbar(config) {
    ...
    forEachBrowserWindow(function(window) {
        ...
        for (let toolbar of window.document.getElementsByTagNameNS(NS_XUL, 'toolbar')) {
            ...
            if (~index) { // Toolbar contains widget...
                ...
                hideButtonFromToolbar(config, window.document);
                return;
            }
        }
        ...
        if (cachedWidgetState && !forceMove && !sstorage._toolbarwidgetState[toolbarWidgetId].hidden) {
    ...
    function hideButtonFromToolbar(config, doc) {
        let { toolbarID, insertbefore, widgetId, toolbarWidgetId, forceMove, weakmap } = config;
        
        // If the user dragged the button back into the palette in a previous session, 
        // we'll need to put it back. We could remove it with currentSet manipulation, 
        // but it will appear in the add-on bar at the bottom. Instead we'll need to 
        // manipulate the XUL by hand.
        if (sstorage._toolbarwidgetState && sstorage._toolbarwidgetState[toolbarWidgetId] && sstorage._toolbarwidgetStat
            // setTimeout ensures that the widget XUL is in the DOM before executing
            // the removal code.
            var tmr = require("sdk/timers");
            tmr.setTimeout(function() {
                let toolbox = doc.getElementById("navigator-toolbox");
                let palette = toolbox.palette;
                let buttonXul = doc.getElementById(widgetId);
                console.error(buttonXul);
                palette.appendChild(buttonXul);
            }, 0);
        }
    }
